...
**Our first independent judge scored everything zero.** That was our fault, not the judge's. We had given it only the clauses as context, and asked it to grade the proposed rewrite as if it were a factual claim. Fixing it meant handing the judge the agent's real inputs and scoring only the explanation. The lesson stuck: an evaluation you have not debugged is not evidence.
